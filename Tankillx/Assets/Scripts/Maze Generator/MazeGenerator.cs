...
public class MazeGenerator : MonoBehaviour
{
    ...
    [Tooltip("How many cells with random removal walls")]
    ...
    {
        int i = 0, randomRange = Random.Range(0, allCells.Count);
        foreach (KeyValuePair<Vector2, Cell> cell in allCells)
        {
            if (i++ == randomRange)
            {
                //cell.Value.spriteRenderer.color = Color.blue;
                return cell.Value.cellObject.transform.position;
            }
        }
        return new Vector3();
    }

    public void RandomRemoveWalls(int n)
    {
        List<Cell2> edgeCells = new List<Cell2>();

        foreach (KeyValuePair<Vector2, Cell> cell in allCells)
        {
            if(cell.Key.x != 1 && cell.Key.y != 1 && cell.Key.x != mazeColumns && cell.Key.y != mazeRows)
            {
                edgeCells.Add(new Cell2 { key = cell.Key, cell = cell.Value });
            }
        }

        for (int i = 0; i < n; i++)
        {
            int randomCell = Random.Range(0, edgeCells.Count);
            Cell newCell = edgeCells[randomCell].cell;
            Vector2 newCellKey = edgeCells[randomCell].key;

            //newCell.spriteRenderer.color = Color.red;
            int wallID = Random.Range(1, 4);
            RemoveWall(newCell.cScript, wallID);
            RemoveNeighbourWall(newCellKey, wallID);
        }
    }

    public void RemoveNeighbourWall(Vector2 oldCell, int wallID)
    {
        foreach(KeyValuePair<Vector2, Cell> newCell in allCells)
        {
            if(wallID == 1) // L
            {
                if(oldCell.x - 1 == newCell.Key.x && oldCell.y == newCell.Key.y)
                {
                    newCell.Value.cScript.wallR.SetActive(false);
                    return;
                }
            }
            ...
            else if (wallID == 2) // R
            {
                if (oldCell.x + 1 == newCell.Key.x && oldCell.y == newCell.Key.y)
                {
                    newCell.Value.cScript.wallL.SetActive(false);
                    return;
                }
            }
            else if (wallID == 3) // U
            {
                if (oldCell.x  == newCell.Key.x && oldCell.y + 1 == newCell.Key.y)
                {
                    newCell.Value.cScript.wallD.SetActive(false);
                    return;
                }
            }
            else if (wallID == 4) // D
            {
                if (oldCell.x == newCell.Key.x && oldCell.y - 1 == newCell.Key.y)
                {
                    newCell.Value.cScript.wallU.SetActive(false);
                    return;
                }
            }
        }
    ...
    public void GenerateCell(Vector2 pos, Vector2 keyPos)
    {
        ...
        // Get reference to attached SpriteRenderer.
        newCell.spriteRenderer = newCell.cellObject.GetComponent<SpriteRenderer>();
